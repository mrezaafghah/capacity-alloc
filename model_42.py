...
def revenue_cal(
    ...
    threshold = 0.3144  # the initial threshold of spot instances
    ...
    for i in range(intervals):
        ...
        spot_price = np.random.normal(loc=threshold+0.024, scale=0.03, size=demand_s[i])
        ...
        for u in range(demand_s[i]):
            ...
            spot_pool.append([spot_price[u], temp2, spot_price[u]*temp2])

        # sorting the spot pool decreasingly (based on the expected revenue)
        spot_pool = sorted(spot_pool, key=itemgetter(2), reverse=True)
